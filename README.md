...
![Tests](https://img.shields.io/badge/tests-210_unit%2Fintegration_%2B_14_e2e-16a34a?style=flat-square)
...
Prefer running it locally? See [Getting started](#getting-started). Prefer proof over clicking? Run `npm test` for the 210-test suite.
...
**210 unit and integration tests plus 14 end-to-end tests**, layered so each level catches what the others cannot. The full plan with per-use-case tables lives in [specs/06-test-plan.md](specs/06-test-plan.md).
...
```bash
npm test          # 210 unit + integration tests
