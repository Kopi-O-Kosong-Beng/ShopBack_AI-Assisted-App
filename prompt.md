...
One more thing: after the core app is done and verified, if we still have enough time, I will tell you the additional advanced features I want to add to make the app stand out. For example, moving the storage from localStorage to SQLite, a login feature so each person has their personal account and their tasks are stored and retrieved per account, XP for each account with a ranking of all users and their department to motivate them and make the app feel tailored for ShopBack, a mascot that shows emotion according to your task load with a cortisol level bar, a calendar view to see when your to-dos are due, and a quick onboarding guide for using the app. When we reach there, update the relevant specs documents and test cases first to keep everything consistent, then implement.
